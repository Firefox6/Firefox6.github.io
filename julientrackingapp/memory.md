...
## Tageseinträge bearbeiten/teilen/importieren (2026-07-29)

Tagesspezifische Kalorieneinträge (nicht Presets) haben jetzt drei Listen-Aktionen statt nur "Löschen": **Bearbeiten**, **Löschen**, **Eintrag teilen** (`renderFoodEntriesForDate`, app.js). "Bearbeiten" nutzt `state.foodEntryEditId` und hijacked das Schnelleintrag-Formular (`renderQuickFoodForm`/`saveQuickFood`, gleiches Muster wie `foodPresetEditId`/`renderFoodPresetForm` für Presets) — Menge/Einheit/`preset_id`/`created_at` bleiben beim Bearbeiten unverändert erhalten, nur Name/Mahlzeit/Nährwerte/Notiz sind editierbar.

"Eintrag teilen" kodiert den kompletten Eintrag (alle Nährwerte + Notiz, ohne `id`/`date`/Timestamps) als JSON `{type:"fittrack_food_entry", v:1, ...}` in einen QR-Code (`shareFoodEntry`/`openShareOverlay`, neues `#share-overlay` in index.html). QR-Rendering nutzt **`qrcodejs` (davidshimjs) via jsDelivr** (`https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js`, globales `window.QRCode`, `new QRCode(container, {text, width, height, correctLevel})`) — **nicht** das npm-Paket `qrcode` (dessen aktuelle Versionen keinen `build/qrcode.min.js`-UMD-Bundle mehr im Package haben, nur noch rohes CommonJS unter `lib/`, das ohne Bundler in einem `<script>`-Tag mit `require is not defined` crashen würde; via jsDelivr-Versionspin 404en diese Pfade auch zuverlässig, nur der unversionierte `/npm/qrcode/...`-Alias liefert zufällig noch was aus einem alten Cache).

Ein 5. Button "📷 Eintrag importieren" (neben Barcode Scannen) öffnet dieselbe Kamera-Overlay-Infrastruktur (`openBarcodeOverlay(mode)`, jetzt mit `mode: "barcode" | "import"` statt fest verdrahtet), scannt aber `qr_code` statt EAN/UPC-Formate. Bei Erkennung parst `handleImportQrDetected` das JSON, validiert `type === "fittrack_food_entry"` und zeigt ein Kontrollformular (`renderImportEntryForm`, wie beim Barcode-Scan) vor dem eigentlichen Speichern (`saveImportedFood`) — kein Blind-Import ohne Bestätigung.
