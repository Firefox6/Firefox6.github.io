import { profile, today, series, trendCards, workouts, sleepTimeline } from "./data/demo-data.js";
...
function renderToday() {
  ...
  return `<div class="view view-today enter">
    <section class="recovery-hero">
      <div class="hero-copy">
        <div class="eyebrow-row"><p class="eyebrow">Recovery</p><span class="confidence">${icon("check")} Hohe Sicherheit</span></div>
        <div class="hero-score-mobile">${scoreRing(today.recovery)}</div>
        <h1><span>${today.recovery}</span>${scoreState(today.recovery).label}</h1>
        <p class="hero-explanation">Deine HRV liegt im persönlichen Normalbereich. Der leicht erhöhte Ruhepuls begrenzt deine Recovery etwas.</p>
        <div class="hero-meta"><span>↑ ${today.delta} zu gestern</span><span>Relevant seit 06:41</span></div>
      ...
      <div class="hero-orbit"><span>28</span><small>Tage Baseline</small></div>
    </section>
    ...
    <div class="today-columns lower">
      ...
      <section class="last-workout">
        ${sectionTitle("Automatisch importiert", "Letztes Training", `<button class="text-button compact" data-action="workout" data-workout="strength">Workout ansehen ${icon("chevron")}</button>`)}
        <div class="workout-summary"><div class="workout-symbol">${icon("activity")}</div><div><strong>Krafttraining</strong><span>Gestern · 58 Minuten</span></div><div class="workout-stats"><strong>Load 42</strong><span>Ø 121 bpm</span></div></div>
      </section>
    </div>
...
function detailHeader(name, value, type) {
  ...
  return `<div class="detail-intro enter"><button class="back-button" data-action="back">${icon("arrow")} Heute</button><div class="detail-score"><div><p class="eyebrow">${name}</p><h1>${value} <span>· ${status.label}</span></h1><p>Heute im Vergleich zu deiner persönlichen 28-Tage-Baseline.</p></div>${scoreRing(value, type)}</div></div>`;
}

function renderRecovery() {
  return `<div class="view detail-view">
    ${detailHeader("Recovery", today.recovery, "recovery")}
    <section class="chart-panel enter delay-1"><div class="chart-panel-head"><div><h2>Recovery-Verlauf</h2><span>7 Tage · Baseline 64–76</span></div><span class="positive-change">↑ 6 zu gestern</span></div>${lineChart(series.recovery, "great", "Recovery der vergangenen sieben Tage")}<div class="chart-axis"><span>Di</span><span>Mi</span><span>Do</span><span>Fr</span><span>Sa</span><span>So</span><span>Heute</span></div><div class="segmented mini"><button class="is-active">7 Tage</button><button>28 Tage</button><button>3 Monate</button><button>6 Monate</button></div></section>
    <section class="detail-grid enter delay-2"><div><div class="section-heading"><div><p class="eyebrow">Nachvollziehbar</p><h2>Faktoren</h2></div><span class="data-quality">${today.completeness}% vollständig</span></div><div class="factor-list expanded">${today.factors.map((factor) => factorRow(factor, true)).join("")}</div></div><aside class="explain-panel"><p class="eyebrow">Deine Berechnung</p><h3>Hohe Datensicherheit</h3><p>HRV, Ruhepuls, Schlaf und die letzte Belastung waren für die Berechnung verfügbar.</p><div class="weight-bars"><span style="--w:35%">HRV <b>35 %</b></span><span style="--w:25%">Schlaf <b>25 %</b></span><span style="--w:20%">Ruhepuls <b>20 %</b></span><span style="--w:20%">Belastung <b>20 %</b></span></div><small>Algorithmus v1 · 3. Aug., 06:43</small></aside></section>
  </div>`;
}
...
function renderSleep() {
  return `<div class="view detail-view">
    ...
    <section class="sleep-metrics enter delay-1">${metricBlock("Geschlafen", "7 h 43", "22 min unter Bedarf")}${metricBlock("Schlafbedarf", "8 h 05", "aus Basis & Belastung")}${metricBlock("Regelmässigkeit", "86 %", "im gewohnten Bereich")}${metricBlock("Unterbrechungen", "3", "kurz, gemessen")}</section>
    <section class="sleep-panel enter delay-2"><div class="section-heading"><div><p class="eyebrow">Letzte Nacht</p><h2>Schlafzeitlinie</h2></div><span class="measured">${icon("check")} Gemessene Phasen</span></div><div class="sleep-times"><span>22:58</span><span>06:52</span></div><div class="sleep-track">${sleepTimeline.map(([from, to, kind]) => `<span class="sleep-stage ${kind}" style="left:${from}%;width:${to - from}%" title="${kind}"></span>`).join("")}</div><div class="sleep-legend"><span><i class="awake"></i>Wach</span><span><i class="light"></i>Leicht</span><span><i class="deep"></i>Tief</span><span><i class="rem"></i>REM</span></div></section>
    <section class="detail-grid sleep-bottom enter delay-3"><div class="chart-panel"><div class="chart-panel-head"><div><h2>Sleep-Verlauf</h2><span>7 Tage · Schlafscore</span></div><span class="positive-change">↑ 2 zu gestern</span></div>${lineChart(series.sleep, "great", "Sleep score Verlauf")}</div><aside class="explain-panel"><p class="eyebrow">Schlafbedarf</p><h3>8 h 05 min</h3><p>Dein Bedarf ergibt sich aus persönlichem Basisbedarf, bestehendem Schlafdefizit und vorheriger Belastung.</p><small>Schlafphasen werden nur gezeigt, wenn Health Connect sie liefert.</small></aside></section>
  </div>`;
}
...
function renderLoad() {
  return `<div class="view detail-view">
    ...
    <section class="load-hero enter delay-1"><div><p class="eyebrow">Heute</p><h2>Moderat belastet</h2><p>Empfohlener Bereich heute: <strong>35–58</strong></p></div><div class="load-scale"><span class="target-range"></span><i style="left:36%"></i><div><small>leicht</small><small>moderat</small><small>hoch</small><small>sehr hoch</small></div></div></section>
    <section class="detail-grid enter delay-2"><div class="chart-panel"><div class="chart-panel-head"><div><h2>Verlauf über den Tag</h2><span>Aktivität und Workouts</span></div><strong>36 Load</strong></div>${lineChart([1, 2, 3, 4, 7, 8, 7, 13, 18, 25, 31, 36], "reduced", "Aufbau der Tagesbelastung")}<div class="activity-markers"><span>06:00</span><b>Spaziergang</b><span>Jetzt</span></div></div><aside class="load-components"><p class="eyebrow">Bestandteile</p><div><span>Alltagsbewegung</span><b>12</b></div><div><span>Workout-Belastung</span><b>8</b></div><div><span>Aktive Zeit</span><b>7</b></div><div><span>Restbelastung</span><b>9</b></div></aside></section>
    <section class="week-load enter delay-3"><div><p class="eyebrow">Diese Woche</p><h2>Im normalen Bereich</h2></div>${metricBlock("Aktuelle Belastung", "214", "bis jetzt")}${metricBlock("Gewohnter Bereich", "180–240", "deine Baseline")}</section>
  </div>`;
}
...
function renderTrends() {
  return `<div class="view view-trends enter">
    ...
    <section class="weekly-review"><div class="weekly-copy"><p class="eyebrow">Automatischer Rückblick</p><h2>Deine Woche</h2><p>Du warst stärker belastet als in der Vorwoche, während deine durchschnittliche Recovery stabil blieb.</p></div><div class="week-numbers"><div><span>Recovery</span><strong>Ø 74</strong><b>+5</b></div><div><span>Sleep</span><strong>Ø 81</strong><b>+3</b></div><div><span>Load</span><strong>236</strong><b>+18 %</b></div></div></section>
  </div>`;
}
...
function renderSettings() {
  ...
  return `<div class="view view-settings enter"><div class="page-heading"><div><p class="eyebrow">FitTrack</p><h1>Einstellungen</h1><p>Deine Datenquellen und App-Verbindung.</p></div></div>
    <section class="settings-section"><div class="settings-head"><div><p class="eyebrow">Datenquellen</p><h2>Verbindungen</h2></div><button class="outline-button" data-action="sync" ${running ? "disabled" : ""}>${icon("sync")}${running ? "Wird aktualisiert" : "Jetzt synchronisieren"}</button></div>${settingRow("Health Connect", isNative ? "Verbindung prüfen" : "Nur in Android-App", `<button data-action="permissions">Verwalten</button>`, !isNative)}${settingRow("Garmin Connect", "Daten erkannt")}${settingRow("FitTrack Konto", "Demo-Modus", `<button data-action="show-login">Anmelden</button>`)}${settingRow("Gemeinsames Gewicht", "Wird intern synchronisiert")}</section>
    <section class="settings-section"><div class="settings-head"><div><p class="eyebrow">Berechtigungen</p><h2>Gesundheitsdaten</h2></div></div><div class="permission-grid">${[["Schlaf", "Erlaubt"], ["Herzfrequenz und HRV", "Erlaubt"], ["Aktivität", "Erlaubt"], ["Workouts", "Erlaubt"], ["Historische Daten", "Erlaubt"], ["Hintergrundzugriff", "Nicht verwendet"]].map(([label, value]) => `<div><span>${label}</span><b class="${value === "Erlaubt" ? "allowed" : "neutral"}">${value === "Erlaubt" ? icon("check") : ""}${value}</b></div>`).join("")}</div></section>
...
function renderSideStatus() {
  return `<aside class="context-rail"><div class="rail-card current"><p class="eyebrow">Heute</p><strong>Gute Erholung</strong><span>78 Recovery</span><div class="rail-bars"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div></div><div class="rail-card"><p class="eyebrow">Deine Baseline</p><strong>Vollständig</strong><span>28 von 28 Tagen</span><div class="baseline-progress"><i></i></div></div><div class="rail-note">${icon("lock")} Deine Gesundheitsdaten bleiben auf deinem Gerät.</div></aside>`;
}
...
function renderModal() {
  if (state.modal === "workout") {
    ...
    return `<div class="modal-backdrop" data-action="close-modal"><section class="sheet workout-sheet" role="dialog" aria-modal="true" aria-label="Workout Details" onclick="event.stopPropagation()"><button class="close-button" data-action="close-modal">${icon("close")}</button><div class="sheet-hero">${workoutTypeIcon(workout.type)}<p class="eyebrow">${workout.day} · ${workout.time}</p><h2>${workout.type}</h2><p>${workout.duration} · Load ${workout.load}</p></div><div class="workout-kpis">${metricBlock("Aktive Kalorien", `${workout.calories}`, "kcal")}${metricBlock("Distanz", workout.distance, "gemessen")}${metricBlock("Ø Herzfrequenz", `${workout.averageHr}`, "bpm")}${metricBlock("Maximum", `${workout.maxHr}`, "bpm")}</div><section class="workout-interpretation"><p class="eyebrow">Einordnung</p><h3>${workout.contribution}</h3><p>Deine Herzfrequenz war während eines grossen Teils des Trainings moderat erhöht. Das Workout trägt ${workout.load} Punkte zu deiner heutigen Load bei.</p></section><section class="zones"><div class="section-heading"><div><p class="eyebrow">Herzfrequenz</p><h3>Zonen</h3></div><span>${workout.duration}</span></div><div class="zone-bars">${workout.zones.map((zone, index) => `<div><span>Zone ${index + 1}</span><i style="--zone:${Math.max(zone, 3)}%"></i><b>${zone} min</b></div>`).join("")}</div></section></section></div>`;
  }
...
function renderOnboarding() {
  const steps = [
    ...
    { eyebrow: "FitTrack wird vorbereitet", title: "Deine erste Auswertung ist bereit.", text: "Health Connect verbunden · Schlafdaten gefunden · Herzfrequenzdaten gefunden · Persönliche Baseline aufgebaut.", action: "Zur Heute-Ansicht", art: "ready" }
  ];
...
async function synchronize() {
  ...
  state.sync = "syncing";
  render();
  const result = await healthAdapter.synchronize();
  state.sync = "current";
  render();
  if (result.reason === "browser") showToast("Browsermodus: lokale Demo- und Cache-Daten bleiben verfügbar.");
  else if (result.reason === "RECORD_READER_NOT_AVAILABLE") showToast("Berechtigungen geprüft. Der aktuelle APK-Bridge stellt noch keinen Messdaten-Leser bereit.");
  else if (!result.updated) showToast("Health Connect konnte nicht erreicht werden. Vorhandene Daten bleiben verfügbar.");
  else showToast("Daten aktualisiert.");
}
...
async function progressOnboarding(alternate = false) {
  ...
  if (state.onboardingStep === 3 && !alternate) await healthAdapter.requestPermissions(["sleep", "recovery", "activity", "workouts"], true);
